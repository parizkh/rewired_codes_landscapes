---
title: "GB1, Ostrov codes, landscape ruggedness"
author: "Hana"
date: "6/15/2022"
...
(corTest = cor.test(tmp_results$PropAccPaths, tmp_results$Robustness))
```

The plot:
```{r}
ggplot(tmp_results, aes(x=Robustness, y=PropAccPaths)) +
  geom_bin2d(bins=c(22,30), alpha=0.8) +
  scale_fill_continuous(type = "viridis", name="Count") +
  theme_test() +
   theme(legend.position="bottom") +
  geom_point(x=rob_standard, y=acc_standard, fill="gray", pch=21, size=2) + 
  labs(x="Code robustness", y="Probability that a randomly\nchosen path to the summit is accessible") +
  geom_smooth(color="black") +
  geom_text(label=paste0("R = ", signif(corTest$estimate, digits=3)), x=0.39, y=max(tmp_results$PropAccPaths)-0.03*(max(tmp_results$PropAccPaths)-min(tmp_results$PropAccPaths)), size=4) +
  annotate(geom = "label", x=rob_standard, y=acc_standard-0.07*(max(tmp_results$PropAccPaths)-min(tmp_results$PropAccPaths)), label="SGC", alpha=0.8, size=3)
```
...
ggplot(data, aes(x=Robustness, y=Correlation, color=NumSplit, shape=pval_cat)) +
  geom_point() + theme_test() +
  labs(y="Correlation between number of stop codons\nand mutational accessibility of the summit", x="Code robustness", main="GB1") +
  scale_color_viridis_d() + geom_hline(yintercept=0, linetype="dashed") +
  scale_shape_discrete(name="P-value")
```
...
data = data[!is.na(data$Correlation),]
 ggplot(data, aes(x=Robustness, y=Correlation, color=NumSplit, shape=pval_cat)) +
  geom_point() + theme_test() +
  labs(y="Correlation between number of stop codons\nand prevalence of no epistasis", x="Code robustness", main="GB1") +
  scale_color_viridis_d() + geom_hline(yintercept=0, linetype="dashed") +
  scale_shape_discrete(name="P-value")
```
...
ggplot(data, aes(x=Robustness, y=Correlation, color=NumSplit, shape=pval_cat)) +
  geom_point() + theme_test() +
  labs(y="Correlation between number of stop codons\nand prevalence of magnitude epistasis", x="Code robustness", main="GB1") +
  scale_color_viridis_d() + geom_hline(yintercept=0, linetype="dashed") +
  scale_shape_discrete(name="P-value")
```
...
ggplot(data, aes(x=Robustness, y=Correlation, color=NumSplit, shape=pval_cat)) +
  geom_point() + theme_test() +
  labs(y="Correlation between number of stop codons\nand prevalence of simple-sign epistasis", x="Code robustness", main="GB1") +
  scale_color_viridis_d() + geom_hline(yintercept=0, linetype="dashed") +
  scale_shape_discrete(name="P-value")
```
...
ggplot(data, aes(x=Robustness, y=Correlation, color=NumSplit, shape=pval_cat)) +
  geom_point() + theme_test() +
  labs(y="Correlation between number of stop codons\nand prevalence of reciprocal-sign epistasis", x="Code robustness", main="GB1") +
  scale_color_viridis_d() + geom_hline(yintercept=0, linetype="dashed") +
  scale_shape_discrete(name="P-value")
```
